...
**Objectif secondaire** :  Donner un apperçu des capacités SIG de R 
...
Si vous avez pas mal de RAM (8Go) et que vous décidez d'afficher quand même l'objet, cela se fait en éxécutant la ligne suivante : 
...
df_GE$nom_region <-  as.character(df_GE$nom_region)

p1 <-  ggplot()+
  geom_sf(data=df_GE ,aes(fill=nom_region %>% as.character()), size=0.1)+
  scale_fill_brewer(type="qual", name = "Valeur brute de \n l'attribut nom_region") +
  theme_void()
p1
``` 
...
Visiblement l'attribut `nom_region` est assez peu informatif et mal renseigné : tout est dans la région Grand Est, il y a des doublons et certaines valeurs sont le nom de régions datant d'avant la réforme territoriale.
On va cartographier les IRIS en les colorant par département
...
Nous allons encore réduire la taille de notre échyantillon pour ne garder que les IRIS du département de la Meuse.
...
Cette cfonction ne fonctionne pas si des polygonjs sont vides, ce qui est le cas de quelques polygon du dataframe `df_55`.
On les enlèves par filtrage en utilisant le prédicat `st_is_empty()`
