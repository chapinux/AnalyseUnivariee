...
Étapes :
  ...
  - calculer le nombre d'arbres par quartier
  - stocker le résultat dans une variable _nb_arbres_ dans la couche vectorielle des quartiers

 
 
 Les packages `dplyr` et `sf`  sont requis , pour les installer , utiliser la fonction `install.packages`.


Fonctions utiles:

- `read_sf` pour charger les données spatiales  
- `plot`  pour afficher 
...
## Comparaison des variables `nombre` et `densité` 
...
Réaliser une cartographie des résidus  du modèle linéaire  
