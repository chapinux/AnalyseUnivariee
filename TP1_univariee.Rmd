...
On peut alors compter le nombre d'arbres par quartiers avec la fonction `table` qui compte (entre autres) le nombre d'individus (ligne) par valeurs distinctes d'une de ses variables. On peut inclure deux (ou plus) variables pour obtenir des tables de contingence.
...
Avant de cartographier cette valeur, regardons sa distribution à l'aide d'un histogramme pour déterminer si un traitement particulier doit être envisagé (en cas de distribution particulièrement biscornue)
...
# la syntaxe est particulière, heureusement c'est assez rare de devoir procéder ainsi !
...
rasthex <-  st_intersection(st_geometry(quartiers), rasthex)
