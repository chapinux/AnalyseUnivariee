...
Nous avons récupéré le fichier des données spatialisé sous le format GeoJSON. D'autres formats sont disponibles, notamment via l'API du site.
...
C'est un dataframe particulier puisqu'il est également de la classe `sf` : l'un de ses colonnes est dédié à la description de sa géométrie (ici, des points 2D)
...
- `levles()` qui donnent les modalités que peut prendre une variable factorielle
...
On peut maintenant tracer les positions des  arbres de Paris intra-muros, après avoir vérifié le CRS de cette couche par la fonction `st_crs()`
